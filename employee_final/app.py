...
def admin_dashboard():
    ...
    # Active Employees
    cursor.execute("""
        SELECT COUNT(DISTINCT TRY_CAST(value AS INT))
        FROM projects
        CROSS APPLY STRING_SPLIT(assigned_to, ',')
    ...
    # Available Projects
    cursor.execute("SELECT COUNT(*) FROM projects WHERE status = 'Available'")
    available_projects = cursor.fetchone()[0]

    # Notifications (example from attendance trigger or alerts)
    cursor.execute("""
        SELECT TOP 5 message, created_at
        FROM notifications
...
def get_notifications():
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, message FROM notifications WHERE employee_id = ? AND is_read = 0", (session['user_id'],))
    notifications = [{"id": row.id, "message": row.message} for row in cursor.fetchall()]
    conn.close()
    return jsonify(notifications)
